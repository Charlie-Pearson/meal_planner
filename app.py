...
class MealPlan(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "day", "meal_type", "account_id", name="unique_day_meal_account"
        ),
    )
...
def get_distinct_aisles() -> List[str]:
    ...
    aisle_names = (
        db.session.query(Aisle.name)
        .filter(Aisle.name.isnot(None), Aisle.name != "")
        .distinct()
        .all()
    )
    ...
    for entry in MealPlan.query.filter_by(account_id=account_id).all():
        ...
        plan[day][meal_type] = {
            ...
            "status": (
                ...
                else ("locked" if entry.is_locked else "new")
            ),
            ...
            "leftover_from": (
                f"{entry.leftover_from_day}_{entry.leftover_from_meal_type}"
                if entry.is_leftover
                and entry.leftover_from_day
                and entry.leftover_from_meal_type
                else None
            ),
        }
    app.logger.info(
        f"Retrieved meal plan for account {account_id} with {len(plan)} days"
    )
    return plan
...
def update_meal_plan(
    ...
    leftover_from_day: Optional[str] = None,
    leftover_from_meal_type: Optional[str] = None,
) -> None:
    """
    ...
    try:
        ...
        entry = MealPlan.query.filter_by(
            account_id=account_id, day=day, meal_type=meal_type
        ).first()
        ...
        if entry:
            ...
            entry = MealPlan(
                ...
                leftover_from_meal_type=(
                    leftover_from_meal_type if is_leftover else None
                ),
            )
        ...
        db.session.commit()
        app.logger.info(
            f"Updated meal plan for {day} {meal_type}: recipe_id={recipe_id}, locked={is_locked}"
        )
    except Exception as e:
        db.session.rollback()
    ...
    if unique_recipe_ids:
        ...
        recipes = (
            Recipe.query.options(
                db.joinedload(Recipe.recipe_ingredients).joinedload(
                    RecipeIngredient.ingredient
                )
            )
            .filter(Recipe.id.in_(unique_recipe_ids))
            .all()
        )
        ...
        for recipe in recipes:
            for recipe_ingredient in recipe.recipe_ingredients:
                ...
                try:
                    ingredient_map[key]["quantity"] += float(
                        recipe_ingredient.quantity or 0
                    )
...
def toggle_meal_lock():
    ...
    try:
        ...
        entry = MealPlan.query.filter_by(
            account_id=account.id, day=day, meal_type=meal_type
        ).first()
        ...
        if entry:
            ...
            # Update session
            if (
                "meal_plan" in session
                and day in session["meal_plan"]
                and meal_type in session["meal_plan"][day]
            ):
            ...
            return (
                jsonify({"success": False, "error": "Meal plan entry not found"}),
                404,
            )
...
def generate_meal_plan(
    ...
    plan_ids: PlanIdsDict = {day: {m: None for m in meal_types} for day in days}

    all_recipes = Recipe.query.all()
    recipes_by_type = {
        m: [r for r in all_recipes if getattr(r, f"is_{m.lower()}")] for m in meal_types
    }
    ...
    def can_fit_leftovers(extra: int, start_idx: int, m_type: str) -> bool:
        if extra <= 0:
            return True
        available = 0
        for idx in range(start_idx + 1, len(days)):
            next_day = days[idx]
            if f"{next_day}_{m_type}" in locked_meals:
                continue
            available += 1
        reserved = sum(item["remaining"] for item in leftover_queues[m_type])
        return extra <= max(0, available - reserved)

    for m_type in meal_types:
        for day_idx, day in enumerate(days):
            slot_id = f"{day}_{m_type}"
            entry: Optional[MealInfoDict] = None
            default_id = default_meals.get(m_type)
            if default_id:
                entry = {"recipe_id": int(default_id), "status": "default"}

            lock_info = locked_meals.get(slot_id)
            locked = False
            if lock_info:
                locked = True
                rid = lock_info.get("recipe_id")
                if rid == -1:
                    entry = {
                        "recipe_id": -1,
                        "status": "locked",
                        "manual_text": lock_info.get("manual_text", "Manual Entry"),
                    }
                else:
                    entry = {"recipe_id": rid, "status": "locked"}

            if not locked and leftover_queues[m_type]:
                lft = leftover_queues[m_type][0]
                entry = {
                    "recipe_id": lft["recipe_id"],
                    "status": "leftover",
                    "leftover_from_day": lft["from_day"],
                    "leftover_from_meal": m_type,
                }
                lft["remaining"] -= 1
                if lft["remaining"] == 0:
                ...
                for r in candidates:
                    extra = (
                        (r.servings // num_people) - 1
                        if r.servings and r.servings > num_people
                        else 0
                    )
                    if can_fit_leftovers(extra, day_idx, m_type):
                        chosen = r
                        break
                if chosen:
                    entry = {"recipe_id": chosen.id, "status": "new"}
                    extra = (
                        (chosen.servings // num_people) - 1
                        if chosen.servings and chosen.servings > num_people
                        else 0
                    )
                    if extra > 0:
                        leftover_queues[m_type].append(
                            {
                                "recipe_id": chosen.id,
                                "remaining": extra,
                                "from_day": day,
            ...
            if (
                entry
                and entry.get("status") in {"default", "locked"}
                and entry.get("recipe_id") != -1
            ):
                recipe = db.session.get(Recipe, entry["recipe_id"])
                extra = (
                    (recipe.servings // num_people) - 1
                    if recipe and recipe.servings and recipe.servings > num_people
                    else 0
                )
                if can_fit_leftovers(extra, day_idx, m_type):
                    if extra > 0:
    ...
    if request.method == "POST":
        ...
        for day in days:
            for meal_type in meal_types:
                ...
                meal_entry = MealPlan.query.filter_by(
                    account_id=account.id, day=day, meal_type=meal_type, is_locked=True
                ).first()
        ...
        try:
            ...
            try:
                ...
                db.session.rollback()
                app.logger.error(
                    f"Error saving meal plan to database: {str(e)}", exc_info=True
                )
...
def add_ingredient_api():
    try:
        ...
        print(
            f"Processing new ingredient - Name: {name}, Unit: {unit}, Aisle: {aisle_name}"
        )  # Debug log
        ...
        try:
            new_ingredient = Ingredient(name=name, unit=unit, aisle_id=aisle.id)
            db.session.add(new_ingredient)
            db.session.commit()
            print(
                f"Successfully created ingredient with ID: {new_ingredient.id}"
            )  # Debug log
...
def add_recipe():
    ...
    distinct_aisles = [
        ...
        if aisle[0] and (isinstance(aisle[0], str) and aisle[0].strip())
    ]
    ...
    if request.method == "POST":
        try:
            ...
            if not servings_str:
                if request.is_json:
                    return (
                        jsonify({"success": False, "message": "Servings is required."}),
                        400,
                    )
            ...
            if not any(
                iid.strip() or name.strip()
                for iid, name in zip(ingredient_ids, ingredient_names)
            ...
            account_id = (
                current_user.accounts.first().id
            ...
            try:
                ...
                for ing_data in ingredients:
                    ...
                    if ing_data["id"] and ing_data["id"].isdigit():
                        ...
                        if not ingredient:
                            raise ValueError(
                                f"Ingredient with ID {ing_data['id']} not found"
                            )
                    else:
            ...
            flash(
                f"An error occurred while processing your request: {str(e)}", "danger"
            )
...
def edit_recipe(recipe_id: int):
    ...
    if not current_user.is_admin and (
        not recipe.account_id
        or not any(acc.id == recipe.account_id for acc in current_user.accounts)
    ):
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            if existing_recipe:
                flash(
                    "A recipe with this name already exists in your account.", "danger"
                )
                errors = True
        ...
        for i in range(max(len(ingredient_ids), len(ingredient_names))):
            ...
            try:
                ...
                flash(
                    f"Invalid quantity for ingredient: {ing_name or 'Unknown'}",
                    "danger",
                )
                errors = True
                break
        ...
        if errors:
            return render_template(
                ...
                or [{"id": "", "name": "", "quantity": "", "unit": "", "aisle": ""}],
                all_ingredients=all_ingredients,
                distinct_aisles=distinct_aisles,
                name=name,
                source_link=source_link,
...
def manage_ingredients():
    ...
    if request.method == "POST":
        try:
            ...
            if new_name:
                ...
                existing = Ingredient.query.filter(
                    or_(
                        ...
                        (
                            func.lower(Ingredient.name) == new_name.lower()[:-1]
                ...
                if existing:
                    flash(
                        f'Ingredient similar to "{new_name}" already exists as "{existing.name}"',
                        "warning",
                    )
                else:
                    new_ingredients.append({"name": new_name, "aisle": new_aisle})
            ...
            for update in updates:
                ...
                if "name" in update and update["name"] != ingredient.name:
                    ...
                    if existing:
                        flash(
                            f"Ingredient \"{update['name']}\" already exists. Skipping update.",
                            "warning",
                        )
                        continue
                ...
                if "unit" in update:
                    new_unit = (
                        update["unit"] or "unit"
                    )  # Default to 'unit' if empty or None
            ...
            if new_ing_name:
                ...
                existing = Ingredient.query.filter(
                    or_(
                        ...
                        (
                            func.lower(Ingredient.name) == new_ing_name.lower()[:-1]
                            if new_ing_name.endswith("s")
                            else None
                        ),
                    )
                ).first()
                ...
                if existing:
                    flash(
                        f'Ingredient similar to "{new_ing_name}" already exists as "{existing.name}"',
                        "warning",
                    )
                else:
                    ...
                    # Create new ingredient
                    ingredient = Ingredient(
                        name=new_ing_name, unit=ingredient_unit, aisle_id=aisle.id
                    )
                    db.session.add(ingredient)
                    db.session.commit()
        ...
        try:
            ingredients = (
                ...
                .outerjoin(Aisle, Ingredient.aisle_id == Aisle.id)
                .order_by(Ingredient.name)
                .all()
            )
    ...
    return render_template(
        "ingredients.html", ingredients=ingredients, distinct_aisles=distinct_aisles
    )
...
def on_join_shopping_list():
    ...
    if not current_user.is_authenticated:
        app.logger.warning(
            f"[WEBSOCKET] Unauthenticated client {request.sid} attempted to join shopping list"
        )
        return
    ...
    try:
        ...
        app.logger.error(
            f"[WEBSOCKET] Error in on_join_shopping_list: {str(e)}", exc_info=True
        )
...
def on_leave_shopping_list():
    ...
    try:
        ...
        app.logger.error(
            f"[WEBSOCKET] Error in on_leave_shopping_list: {str(e)}", exc_info=True
        )
