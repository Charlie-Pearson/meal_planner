...
class User(UserMixin, db.Model):
    ...
    accounts = db.relationship(
        "Account",
        secondary="account_user",
        back_populates="users",
        lazy="dynamic",
        overlaps="account_users,user",
    )
    account_users = db.relationship(
        "AccountUser", back_populates="user", overlaps="accounts,account"
    )
...
class Account(db.Model):
    ...
    account_users = db.relationship(
        "AccountUser", back_populates="account", overlaps="users,user"
    )
...
class AccountUser(db.Model):
    ...
    account = db.relationship(
        "Account", back_populates="account_users", overlaps="users"
    )
    user = db.relationship("User", back_populates="account_users", overlaps="accounts")
...
class LockedMeal(db.Model):
    ...
    )  # Add lock_type field with default 'user'

    __table_args__ = (db.UniqueConstraint("day", "meal_type", name="unique_day_meal"),)
...
class AccountSettings(db.Model):
    ...
    meal_plan_start_day = db.Column(
        db.String(10), default="Monday"
    )  # Monday, Tuesday, etc.
    meal_plan_duration = db.Column(db.Integer, default=7)  # Number of days
    meal_repeat_interval = db.Column(
        db.Integer, default=0
    )  # 0 means no restriction, otherwise number of days
...
def update_pantry(
    item_name: str, quantity: str, unit: str, aisle: Optional[str] = None
) -> None:
    """
    ...
    pantry_item = PantryItem.query.filter(
...
def remove_from_pantry(item_id: int) -> None:
    ...
    if pantry_item:
        ...
        try:
            ...
            app.logger.error(
                f"Error removing pantry item ID {item_id}: {e}"
            )  # Log error
    else:
...
def get_distinct_aisles() -> List[str]:
    ...
    q1 = (
        db.session.query(Ingredient.aisle)
        .filter(Ingredient.aisle.isnot(None), Ingredient.aisle != "")
        .distinct()
    )
    # Query distinct aisles from Pantry
    q2 = (
        db.session.query(PantryItem.aisle)
        .filter(PantryItem.aisle.isnot(None), PantryItem.aisle != "")
        .distinct()
    )
...
def get_persistent_locks() -> Dict[str, Dict[str, Any]]:
    ...
    for lock in LockedMeal.query.all():
        ...
        app.logger.debug(
            f"Retrieved lock from DB: {slot_id} - recipe_id: {lock_info['recipe_id']}, manual: {lock_info['manual']}, default: {lock_info['default']}, lock_type: {lock_info['lock_type']}"
        )
    ...
    try:
        ...
        if lock_info:
            ...
            new_lock = LockedMeal(
                ...
                lock_type=lock_info.get(
                    "lock_type", "user"
                ),  # Add lock type with default 'user'
            )
        ...
        db.session.rollback()
        app.logger.error(
            f"Error updating persistent lock for {slot_id}: {str(e)}", exc_info=True
        )
        raise
...
def sync_session_locks_with_db() -> None:
    ...
    app.logger.info(
        f"Syncing session locks with database. Found {len(db_locks)} locks in database."
    )
...
AggregatedIngredientInfo = Dict[
    str, Any
]  # Contains unit, recipes, aisle, needs_check, required_qty
ShoppingListDict = Dict[str, List[Dict[str, Any]]]  # Aisle -> List of Item Dicts
PlanIdsDict = Dict[
    str, Dict[str, Dict[str, Any]]
]  # day -> meal_type -> recipe_id or manual text
...
def generate_shopping_list_data(plan_ids: PlanIdsDict) -> ShoppingListDict:
    ...
    app.logger.debug(
        f"[SHOPLIST] Called generate_shopping_list_data with plan_ids: {plan_ids}"
    )
    ...
    for day, meals in plan_ids.items():
        for meal_type, meal_info in meals.items():
            if (
                meal_info
                and meal_info.get("recipe_id") not in (None, -1)
                and meal_info.get("status") != "leftover"
            ):
                unique_recipe_ids.add(meal_info["recipe_id"])
    app.logger.debug(
        f"[SHOPLIST] Unique recipe IDs for aggregation: {unique_recipe_ids}"
    )
    # --- 2. Aggregate ingredients ---
    ingredient_map = defaultdict(
        lambda: {"quantity": 0, "unit": None, "aisle": None, "recipes": set()}
    )
    ...
    for (name, unit), data in ingredient_map.items():
        ...
        if (
            pantry_item
            and (pantry_item.unit or "").strip().lower() == (unit or "").strip().lower()
        ):
            try:
            ...
            shopping_list_by_aisle[data["aisle"]].append(
                {
                    ...
                    "pantry_deducted": (
                        min(data["quantity"], pantry_qty) if pantry_qty else 0
                    ),
                }
            )
            app.logger.debug(
                f"[SHOPLIST] Added item: {name}, qty: {remaining_qty}, aisle: {data['aisle']}, unit: {data['unit']}"
            )
    ...
    app.logger.debug(
        f"[SHOPLIST] Final shopping_list_by_aisle: {shopping_list_by_aisle}"
    )
...
def clear_leftover_locks() -> None:
    """Remove leftover locks from the database and session."""
    session_locks = session.get("locked_meals", {})
    leftover_keys = [
        k for k, v in session_locks.items() if v.get("lock_type") == "leftover"
    ]
    for key in leftover_keys:
        session_locks.pop(key, None)
    session["locked_meals"] = session_locks
    session.modified = True
    LockedMeal.query.filter_by(lock_type="leftover").delete()
    db.session.commit()


def apply_manual_leftovers(
    plan_ids: PlanIdsDict,
    locked_meals: LockedMealsDict,
    num_people: int,
    days: List[str],
) -> None:
    """Apply leftover logic for manually added recipes."""
    for slot_id, lock_info in locked_meals.items():
        recipe_id = lock_info.get("recipe_id")
        if not lock_info.get("manual") or not recipe_id or recipe_id <= 0:
            continue
        day, meal_type = slot_id.split("_")
        recipe = db.session.get(Recipe, recipe_id)
        if not recipe:
            continue
        plan_ids.setdefault(day, {})[meal_type] = {
            "recipe_id": recipe_id,
            "status": "locked",
            "locked_by_main": True,
            "manual_text": lock_info.get("text"),
        }
        leftovers = (recipe.servings or 0) - num_people
        if leftovers >= num_people:
            assign_leftovers(
                plan_ids, day, meal_type, leftovers, num_people, recipe_id, days
            )
...
def assign_leftovers(
    ...
    while leftovers >= num_people and next_day:
        ...
        if slot_info is not None:
            if (
                slot_info.get("locked_by_main", False)
                or slot_info.get("locked_by_user", False)
                or slot_info.get("default_lock", False)
            ):
...
def generate_meal_plan(
    ...
    plan_ids: PlanIdsDict = {
        day: {meal_type: None for meal_type in meal_types} for day in days
    }
            ...
            if recipe_id == -1:
                ...
                if db.session.get(Recipe, recipe_id):
                    if lock_info.get("lock_type") == "leftover":
                        ...
                        if (
                            meal_type == "Breakfast"
                            and plan_ids[day][meal_type]
                            and plan_ids[day][meal_type].get("default_lock")
                        ):
                            plan_ids[day][meal_type]["default_lock"] = False
                else:
                    # Locked recipe doesn't exist (maybe deleted)
                    flash(
                        f"Locked recipe ID {recipe_id} for {slot_id} not found in database. Lock ignored.",
                        "warning",
                    )
                    # Ensure this invalid lock is not active and plan slot is cleared if it held the bad ID
                    if (
                        current_coords in active_locks
                        and active_locks[current_coords] == recipe_id
                    ):
                        del active_locks[current_coords]
                    if (
                        plan_ids[day][meal_type]
                        and plan_ids[day][meal_type].get("recipe_id") == recipe_id
                    ):
    ...
    for day_index, day in enumerate(days):
        for meal_type in meal_types:
            ...
            if meal_type == "Breakfast" and default_breakfast_id:
                non_default_breakfasts = [
                    r for r in available_recipes if r.id != default_breakfast_id
                ]
...
def update_shopping_item_checked():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "item_id": item_id, "is_checked": item.is_checked}
        )
    ...
    if not item or item.account_id != account.id:
        app.logger.error(
            f"[DEBUG-update-item] Item not found or access denied for item_id={item_id}"
        )
        return jsonify({"success": False, "error": "Item not found"}), 404
    ...
    db.session.commit()
    print(
        f"[CONSOLE] Shopping list item update: item_id={item_id}, is_checked={item.is_checked}"
    )
    app.logger.info(
        f"[DEBUG-update-item] Updated item_id={item_id} is_checked={is_checked}"
    )
    return jsonify({"success": True})
...
def unlock_all_meals():
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(
            f"Unlock All: Deleted {num_deleted} locked meals for account {account.id}"
        )
        return jsonify({"success": True, "unlocked_count": num_deleted})
    except Exception as e:
        db.session.rollback()
        app.logger.error(
            f"Unlock All: Error unlocking all meals: {str(e)}", exc_info=True
        )
...
def dashboard():
    ...
    settings = getattr(account, "settings", None)
    meal_plan_start_day = (
        getattr(settings, "meal_plan_start_day", "Monday") if settings else "Monday"
    )
    ...
    start_idx = (
        ALL_DAYS.index(meal_plan_start_day) if meal_plan_start_day in ALL_DAYS else 0
    )
    ...
    if request.method == "POST":
        ...
        # Remove leftover locks before regenerating the plan
        clear_leftover_locks()
        ...
        for day in days:
            for meal_type in meal_types:
                ...
                app.logger.debug(
                    f"Processing slot {slot_id}: manual_select={manual_select}, lock_checkbox={lock_checkbox}, recipe_id={recipe_id_in_slot_str}"
                )
                ...
                if manual_select == "-1" and manual_text:
                    ...
                    app.logger.info(
                        f"Setting manual text lock for {slot_id}: {manual_text}"
                    )
                ...
                # 2. Manual Recipe Selection
                    ...
                    try:
                        ...
                        if db.session.get(Recipe, recipe_id):
                            ...
                            app.logger.info(
                                f"Setting manual recipe lock for {slot_id}: {recipe_id}"
                            )
                        else:
                            flash(
                                f"Selected recipe ID {recipe_id} for {slot_id} not found. Selection ignored.",
                                "warning",
                            )
                    except (ValueError, TypeError):
                        flash(
                            f"Invalid recipe selection value '{manual_select}' for {slot_id}. Selection ignored.",
                            "warning",
                        )
                ...
                # 3. Checkbox or Lock All
                    ...
                    app.logger.debug(
                        f"Slot {slot_id} should_lock: {should_lock_this_slot}, lock_checkbox: {lock_checkbox}, lock_all: {lock_all}"
                    )
                    ...
                    if not should_lock_this_slot and not lock_all:
                        ...
                        app.logger.info(
                            f"Removing lock for {slot_id} (checkbox unchecked)"
                        )
                    elif should_lock_this_slot:
                        current_recipe_id: Optional[int] = None
                        try:
                            if (
                                recipe_id_in_slot_str
                                and recipe_id_in_slot_str.isdigit()
                            ):
                                current_recipe_id = int(recipe_id_in_slot_str)
                            elif lock_all:
                                slot_info = plan_ids_before_update.get(day, {}).get(
                                    meal_type, {}
                                )
                            ...
                            flash(
                                f"Invalid recipe ID format '{recipe_id_in_slot_str}' found for {slot_id} during lock.",
                                "warning",
                            )
                        ...
                        if current_recipe_id and current_recipe_id > 0:
                            if db.session.get(Recipe, current_recipe_id):
                                ...
                                app.logger.info(
                                    f"Setting user lock for {slot_id}: {current_recipe_id}, manual=False, lock_type=user"
                                )
                            else:
                                flash(
                                    f"Recipe ID {current_recipe_id} for {slot_id} not found. Lock not applied.",
                                    "warning",
                                )
                                lock_info_to_set = None
        ...
        apply_manual_leftovers(
            plan_ids, session["locked_meals"], session["num_people"], days
        )
    ...
    if "current_plan_ids" not in session:
        ...
        session["current_plan_ids"] = generate_meal_plan(
            num_people, session.get("locked_meals", {}), days=days
        )
        session.modified = True
    ...
    all_recipe_ids_in_plan: Set[int] = {
        ...
        for mi in dp.values()
        if mi
        and mi.get("recipe_id")
        and mi["recipe_id"] != -1  # Check existence and valid ID
    }
    recipes_in_plan_dict: Dict[int, Recipe] = (
        {
            r.id: r
            for r in Recipe.query.filter(Recipe.id.in_(all_recipe_ids_in_plan)).all()
        }
        if all_recipe_ids_in_plan
        else {}
    )
    ...
    active_locked_meals_state = session.get(
        "locked_meals", {}
    )  # Get current lock state for template
    app.logger.debug(
        f"[DASHBOARD] Passing locked_meals to template: {active_locked_meals_state}"
    )
...
def add_recipe():
    ...
    if request.method == "POST":
        try:
            ...
            if (
                name
                and Recipe.query.filter(func.lower(Recipe.name) == name.lower()).first()
            ):
                flash(
                    f"A recipe named '{name}' already exists. Please choose a different name.",
                    "warning",
                )
                errors = True
            ...
            all_ingredient_aisles = (
                db.session.query(Ingredient.name, Ingredient.aisle)
                .filter(Ingredient.aisle.isnot(None), Ingredient.aisle != "")
                .distinct()
                .all()
            )
            known_aisles_cache = {
                ing_name.strip().lower(): aisle
                for ing_name, aisle in all_ingredient_aisles
                if aisle
            }
            ...
            ):  # Use splitlines() handles different line endings
                line = line.strip()
                if not line:
            ...
            if not has_valid_ingredient:
                ...
                db.session.rollback()
                flash(
                    "No valid ingredients found. Each line should be 'Name - Quantity - Unit' (Quantity and Unit are optional). Recipe not added.",
                    "danger",
                )
                return render_template("add_recipe.html", current_data=request.form)
            ...
            flash(
                f"An unexpected error occurred while adding the recipe: {e}", "danger"
            )
            app.logger.error(
                f"Error adding recipe: {e}", exc_info=True
            )  # Log detailed error
...
def edit_recipe(recipe_id: int):
    ...
    def get_submitted_ingredients(form_data) -> List[Dict[str, Any]]:
            ...
            submitted.append(
                {
                    ...
                    "aisle": (
                        aisles[i].strip()
                        if i < len(aisles) and aisles[i].strip() and aisles[i] != "None"
                        else None
                    ),
                }
            )
        ...
        if not submitted:
            submitted.append(
                {"id": "", "name": "", "quantity": "", "unit": "", "aisle": None}
            )
        return submitted
    ...
    if request.method == "POST":
        ...
        try:
            ...
            max_len = max(
                ...
                len(ingredient_aisles),
            )
            for i in range(max_len):
                name_val = (
                    ingredient_names[i].strip() if i < len(ingredient_names) else ""
                )
                ...
                try:
                    current_id_str = (
                        ingredient_ids[i].strip() if i < len(ingredient_ids) else ""
                    )
                ...
                qty_val = (
                    ingredient_qtys[i].strip()
                    if i < len(ingredient_qtys) and ingredient_qtys[i].strip()
                    else None
                )
                unit_val = (
                    ingredient_units[i].strip()
                    if i < len(ingredient_units) and ingredient_units[i].strip()
                    else None
                )
                aisle_val = (
                    ingredient_aisles[i].strip()
                    if i < len(ingredient_aisles)
                    and ingredient_aisles[i].strip()
                    and ingredient_aisles[i] != "None"
                    else None
                )
            ...
            if ids_to_delete:
                ...
                Ingredient.query.filter(Ingredient.id.in_(ids_to_delete)).delete(
            ...
            for ing_id, data in ingredients_to_update.items():
                ...
                if ing:  # Ensure it still exists
                    ...
                    app.logger.warning(
                        f"Ingredient ID {ing_id} marked for update but not found in session/DB."
                    )
            ...
            flash(
                f"An unexpected error occurred while updating the recipe: {e}", "danger"
            )
...
def delete_recipe(recipe_id: int):
    ...
    try:
        ...
        if keys_to_remove:
            ...
            session.modified = True
            flash(
                f"Removed associated meal locks for deleted recipe '{recipe_name}'.",
                "info",
            )
        ...
        db.session.commit()
        flash(
            f"Recipe '{recipe_name}' and its ingredients deleted successfully.",
            "success",
        )
...
def shopping_list():
    ...
    items = (
        ShoppingListItem.query.filter_by(account_id=account.id)
        .order_by(ShoppingListItem.aisle, ShoppingListItem.name)
        .all()
    )
    app.logger.debug(
        f"[DEBUG-shopping-list] Items fetched from DB: {[ (item.id, item.name, item.quantity, item.unit, item.aisle) for item in items ]}"
    )
    ...
    return render_template(
        "shopping_list.html", items_by_aisle=items_by_aisle, aisles=aisles
    )
...
def move_shopping_item():
    ...
    if not request.is_json:
        ...
        return (
            jsonify(success=False, error="Invalid request format, JSON expected."),
            400,
        )
    ...
    is_checked = data.get(
        "isChecked"
    )  # Boolean: True means moving TO removed, False means moving FROM removed
    item_data = data.get(
        "item_data", {}
    )  # Additional data about the item from the client

    if not norm_name:
        app.logger.warning("Missing 'norm_name' in request to /move_shopping_item")
        return (
            jsonify(success=False, error="Missing item identifier ('norm_name')."),
            400,
        )
    ...
    if is_checked:  # --- Moving item TO removed list ---
        if norm_name not in removed_items_dict:
            ...
            item_data_for_client = removed_items_dict.get(
                norm_name
...
def update_shopping_list_aisles(ingredient_name: str, new_aisle: Optional[str]) -> None:
    ...
    ).update({"aisle": new_aisle}, synchronize_session=False)
...
def manage_aisles():
    ...
    if request.method == "POST":
        try:
            ...
            for update in updates:
                ...
                Ingredient.query.filter(Ingredient.name.ilike(update["name"])).update(
                    {"aisle": update["new_aisle"], "updated_at": datetime.utcnow()},
                    synchronize_session=False,
                )

                # Update PantryItems table
                PantryItem.query.filter(PantryItem.name.ilike(update["name"])).update(
                    {"aisle": update["new_aisle"]}, synchronize_session=False
                )
            ...
            flash(
                f"Successfully updated {len(updates)} ingredient aisle assignments.",
                "success",
            )
            return redirect(url_for("manage_aisles"))
        except Exception as e:
    ...
    try:
        ...
        ingredients = (
            db.session.query(Ingredient.name, func.max(Ingredient.aisle).label("aisle"))
...
def cupboard():
    ...
    if request.method == "POST":
        ...
        if delete_id_str:
            ...
            if not add_aisle:
                existing_ing = (
                    db.session.query(Ingredient.aisle)
                    .filter(
                        func.lower(Ingredient.name) == add_name.lower(),
                        Ingredient.aisle.isnot(None),
                        Ingredient.aisle != "",
                    )
                    .first()
                )
                if existing_ing:
                    ...
                    flash(
                        f"Inherited aisle '{add_aisle}' for '{add_name}' from ingredients.",
                        "info",
                    )
        ...
        # Check if it was an 'add' attempt specifically without a name
            ...
            flash(
                "Item name is required to add or update an item in the pantry.",
                "warning",
            )
    ...
    pantry_items_list = PantryItem.query.order_by(
        PantryItem.aisle.asc().nullslast(), func.lower(PantryItem.name)
    ).all()
    ...
    sorted_aisle_keys = sorted(
        ...
        key=lambda a: (
            aisle_order_map.get(a, 999),
            a,
        ),  # Use 999 for unknown/other aisles
    )
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            existing_membership = AccountUser.query.filter_by(
                account_id=account_user.account_id, user_id=existing_user.id
            ).first()
...
def manage_users():
    ...
    pending_invites = Invitation.query.filter_by(
        account_id=account_user.account_id, is_used=False
    ).all()
    ...
    return render_template(
        "manage_users.html", users=users, pending_invites=pending_invites
    )
...
def update_user_role(user_id):
    ...
    account_user = AccountUser.query.filter_by(
        account_id=current_account_user.account_id, user_id=user_id
    ).first()
...
def accept_invitation(token):
    ...
    if request.method == "POST":
        ...
        if not current_user.is_authenticated:
            ...
            if current_user.email != invitation.email:
                flash(
                    "You must be logged in with the email address the invitation was sent to.",
                    "error",
                )
                return redirect(url_for("accept_invitation", token=token))
            user = current_user
    ...
    return render_template(
        "accept_invitation.html", invitation=invitation, now=datetime.utcnow()
    )
...
def resend_invite(invite_id):
    ...
    invitation = Invitation.query.filter_by(
        id=invite_id, account_id=current_account_user.account_id, is_used=False
    ).first()
    ...
    try:
        ...
        invitation_url = url_for(
            "accept_invitation", token=invitation.token, _external=True
        )
        flash(
            f"Invitation resent to {invitation.email}. Invitation link: {invitation_url}",
            "success",
        )
...
def settings():
    ...
    if request.method == "POST":
        try:
            ...
            settings.meal_plan_start_day = request.form.get(
                "meal_plan_start_day", "Monday"
            )
            settings.meal_plan_duration = int(request.form.get("meal_plan_duration", 7))
            settings.meal_repeat_interval = int(
                request.form.get("meal_repeat_interval", 0)
            )

            # Update default meals
            settings.default_breakfast_id = (
                request.form.get("default_breakfast_id") or None
            )
...
def generate_meal_plan_route():
    if request.method == "POST":
        ...
        for key, value in request.form.items():
            if key.startswith("lock_"):
                ...
                if recipe_id == "-1":
                    ...
                    meal_locks[slot_id] = {"recipe_id": int(recipe_id)}
        ...
        try:
            ...
            if not account:
                flash(
                    "No account found. Please create or join an account first.", "error"
                )
                return redirect(url_for("dashboard"))
            ...
            try:
                ...
                flash(
                    "Shopping list may be out of date. Please refresh it manually.",
                    "warning",
                )

            flash(
                "Meal plan generated successfully! Please review and confirm.",
                "success",
            )
...
def generate_meal_plan_post():
    ...
    try:
        ...
        meal_locks = [
            lock for lock in meal_locks if lock.strip()
        ...
        for lock in meal_locks:
            try:
                ...
                recipe = Recipe.query.filter_by(
                    id=recipe_id, account_id=account_id
                ).first()
                if not recipe:
                    continue
            ...
            for meal_type in meal_types:
                ...
                if (current_date, meal_type) in locked_meals:
                    ...
                    available_recipes = [
                        ...
                        for r in recipes
                        if (
                            (meal_type == "Breakfast" and r.is_breakfast)
                            or (meal_type == "Lunch" and r.is_lunch)
                            or (meal_type == "Dinner" and r.is_dinner)
                        )
                    ]
                    if not available_recipes:
                        continue
        ...
        app.logger.debug(
            f"[DEBUG-gmpost] start_date: {start_date}, end_date: {end_date}"
        )
        ...
        app.logger.debug(
            f"[DEBUG-gmpost] meals in DB for meal_plan.id={meal_plan.id}: {[{'id': m.id, 'date': m.date, 'meal_type': m.meal_type, 'recipe_id': m.recipe_id} for m in meals]}"
        )
        for meal in meals:
        ...
        session.modified = True
        app.logger.debug(
            f"[DEBUG-gmpost] session['current_plan_ids']: {session.get('current_plan_ids')}"
        )
        ...
        try:
            ...
            db.session.rollback()
            app.logger.error(
                f"[ERROR-gmpost] Failed to regenerate shopping list: {str(e)}"
            )
            flash(
                "Meal plan generated but shopping list may be out of date.", "warning"
            )
        ...
        app.logger.debug(
            f"[DEBUG-gmpost] Finished generate_shopping_list() call. Shopping list: {shopping_list}"
        )
...
def check_shopping_list_updates():
    ...
    if (
        latest_item
        and latest_item.updated_at
        and latest_item.updated_at.timestamp() > last_update
    ):
...
def regenerate_shopping_list():
    ...
    try:
        db.session.commit()
        flash(
            "Shopping list regenerated successfully with updated aisle assignments.",
            "success",
        )
    except Exception as e:
        db.session.rollback()
